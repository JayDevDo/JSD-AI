...
const openDialDialog = (pos, dial = null) => {
	...
	const slotFree = (tabId, row, col) => {	return !JSDPos.posTaken(JSDStore.makePos(tabId, row, col), oldPos);	};
	...
	applyBtn.addEventListener("click", () => {
		if (tabSelect.value === archTabId) {
			...
			if (res.ok) {JSDStore.commit();	buildTabGrid();	dialog.remove();}
		...
		if (workDial.pinned && (!editMode || !dial.pinned)) {
			...
			if (!plan.ok) {	planMssg.hidden = false;planMssg.textContent = plan.mssgs.join("\n");return;}
