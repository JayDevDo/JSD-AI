...
const archDial = (dial) => {
	...
	appData.allDials = appData.allDials.filter((oldDial) => {
		return oldDial.position.tabId !== archTabId && !JSDStore.samePos(oldDial.position, dial.position);
	});
	...
	appData.allDials = appData.allDials.concat(newArch);
	res.mssgs.push("Archived: " + dial.label);
	if (fifo.length) {
		res.mssgs.push("Archive full, dropped: " + fifo[0].label);
	}
	saveData();
	return res;
};
...
const delToArch = (pos) => {
	...
	if (!dial) {
		return { ok: false, mssgs: ["No dial at selected position"] };
	}
